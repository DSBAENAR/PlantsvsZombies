        ...
        newFolderButton.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor) {
                if (newFolderButton.isChecked()) {
                    ...
                    // Crear un nuevo diálogo para ingresar el nombre de la carpeta
                    NewFileDialog dialog = new NewFileDialog("New Folder", skin) {
                        @Override
                        protected void result(Object object) {
                            final boolean success = (Boolean) object;

                            if (success) {
                                String folderName = getResult();
                                System.out.println(folderName);
                                if (folderName == null || folderName.trim().isEmpty()) {
                                    System.out.println("Error: Nombre de la carpeta no puede estar vacío.");
                                    return;
                                }

                                // Crear la carpeta con el nombre ingresado
                                FileHandle newFolder = new FileHandle(currentDir.path() + "/" + folderName.trim());
                                if (newFolder.exists()) {
                                    System.out.println("Error: El directorio ya existe.");
                                } else {
                                    newFolder.mkdirs();
                                    System.out.println("Carpeta creada: " + newFolder.path());
                                    changeDirectory(currentDir); // Actualizar la vista del directorio
                                }
                            }
                        }
                    };
